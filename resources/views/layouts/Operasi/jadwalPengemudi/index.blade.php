...
@section('content')
    <h3>Jadwal Pengemudi Wisata</h3>

    <div class="col-12">
        <div class="card mb-4">
            <div class="card-body">
                <form method="GET" action="{{ route('pengemudi.show', ['date_start' => $date_start, 'date_end' => $date_end]) }}">
                    @csrf
                    <div class="row">
                        <div class="col-md-6 col-12 mb-4">
                            <label for="start" class="form-label">
                                <a href="{{ route('pengemudi.show', ['date_start' => $date_start, 'date_end' => $date_end]) }}" class="text-lx text-red-700">Tanggal Pemakaian</a>
                            </label>
                        ...
                        {{-- <div class="col-md-2 col-6 mb-4 d-flex align-items-end">
                            <a href="{{ route('jadwalpengemudi.pdf', ['date_start' => $date_start, 'date_end' => $date_end, 'type_id' => request('type_id')]) }}" class="btn btn-secondary w-100 text-nowrap" target="_blank">Download PDF</a>
                        </div> --}}
                        <div class="col-md-2 col-6 mb-4 d-flex align-items-end">
        ...
        <div class="table-container">
            <div class="table-responsive text-nowrap">
                <table class="table table-hover" style="zoom: 0.85">
                    <thead>
                        <tr>
                            <th style="font-size: 14px">No.Pengemudi</th>
                            <th style="font-size: 14px">Nama Pengemudi</th>
                    ...
                    <tbody>
                        @foreach($pengemudis as $pengemudi)
                            <tr>
                                ...
                                <td class="bus-name" style="font-size: 14px">{{ $pengemudi->nopengemudi }}</td>
                                <td class="bus-name" style="font-size: 14px">{{ $pengemudi->users->name }}</td>

                                @foreach($dates as $date => $day)
                                    <td class="{{ $schedule[$pengemudi->id][$date] == 'Standbye' ? 'available' : 'filled' }}"
                                        style="background-color: {{ $schedule[$pengemudi->id][$date] == 'Standbye' ? 'white' : 'red' }};
                                            color: {{ $schedule[$pengemudi->id][$date] == 'Standbye' ? 'green' : 'white' }};
                                            font-size: 14px;">
                                        {!! $schedule[$pengemudi->id][$date] !!}
                                    </td>
                                @endforeach
                            </tr>
                        @endforeach
