...
@section('content')
                ...
                @elseif ($bus->isNotEmpty())
                    <div id="wizard-property-listing" class="bs-stepper vertical mt-2 linear">
                        <div class="bs-stepper-content">
                            <form id="form-update" class="form-horizontal" data-id="{{ $booking->id }}">
                                @csrf
                                                    ...
                                                    <div class="form-check custom-option custom-option-icon">
                                                        <label class="form-check-label custom-option-content"
                                                            ...
                                                            <span class="custom-option-body">
                                                                <i class="bx bx-bus"></i>
                                                                <span class="custom-option-title">{{ $data->nobody }}</span>
                                                                <small>{{ $data->nopolisi }}</small>
                                                            </span>
                                                            <input name="bus_id[]" class="form-check-input" type="checkbox"
                                    ...
                                    <div class="row g-3 mt-3">
                                        ...
                                        <div class="col-sm-6 fv-plugins-icon-container">
                                            ...
                                            <input type="text" id="telephone" name="telephone"
                                                value="{{ $booking->telephone }}" class="form-control">
                                        </div>
                                        <div class="col-sm-6 ">
                                        ...
                                        <div class="col-sm-6 fv-plugins-icon-container">
                                            ...
                                            <input type="text" id="total_harga_std" value="{{ $booking->harga_std }}"
                                                name="harga_std" class="form-control" readonly>
                                        </div>
                                        <div id="selected_tujuan" hidden>{{ $booking->tujuan_id }}</div>

                                        <div class="col-sm-6">
                                            <label class="form-label" for="total_bus">Total Bus</label>
                                            <input type="number" id="total_bus" value="{{ $booking->total_bus }}"
                                                name="total_bus" class="form-control" readonly>
                                        </div>
                                        <div class="col-sm-6">
                                            <label class="form-label" for="grand_total">Total Biaya</label>
                                            <input type="text" id="grand_total" value="{{ $booking->grand_total }}"
    ...
    <script>
        ...
        $(document).ready(function() {
            // Initialize select2 for tujuan_id with default value
            var bookingTujuanId = "{{ $booking->tujuan_id }}"; // Ambil tujuan_id dari $booking
            var bookingTujuanText =
                "{{ $booking->tujuan->nama_tujuan }} - {{ $booking->tujuan->pemakaian }} - {{ $booking->tujuan->typearmadas->name }}"; // Ambil teks tujuan dari $booking

            // Inisialisasi select2 dengan nilai default
            $('#tujuan_id').select2({
            ...
            var option = new Option(bookingTujuanText, bookingTujuanId, true, true);
            $('#tujuan_id').append(option).trigger('change');

            // Event listener untuk perubahan pada select2
            $('#tujuan_id').on('change', function(e) {
                var tujuanId = $(this).val(); // Ambil tujuan_id yang dipilih
                $('#selected_tujuan').text(
                tujuanId); // Tampilkan tujuan_id yang dipilih di elemen dengan id "selected_tujuan"
            ...
            $('#form-update').on('submit', function(e) {
                ...
                $.ajax({
                    url: '/update/' + $(this).data('id'), // Correct URL to include the booking ID
                    method: "POST",
                    headers: {
                    ...
                    data: $(this).serialize(),
                    success: function(response) {
                        alert("Data saved, you will be redirected to detail page");
                        location.reload();
                    },
                    error: function(response) {
                        var response = response.responseJSON;
                        alert(response.error);
                        $("#form-update").find(".submit").removeClass('hidden');
                    }
                });
            });
